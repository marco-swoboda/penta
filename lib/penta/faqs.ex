defmodule Penta.FAQs do
  ...
  def upvote!(%FAQ{} = faq) do
    {1, [%FAQ{vote_count: vote_count}]} =
      from(f in FAQ, where: f.id == ^faq.id, select: [:vote_count])
      |> Repo.update_all(inc: [vote_count: 1])

    put_in(faq.vote_count, vote_count)
  end
end
